...
private:
    ...
    void handleMovement(Vector2D velocity)
    {
        ...
        {
          int total = units_move + m_velocity.m_x; // look ahead

          if( total >= -max_move_x && total <= max_move_x){ // if less than 200, greater than -200
            m_position.m_x = newPos.m_x;
            units_move = units_move + m_velocity.m_x;
          } else{
            if( moveLeft ){
              moveLeft = false;
              moveRight = true;
